...
col_list=['from','to','quarter','kappa','kappa_all','kappa_sole','kappa_soleshared','skappa','skappa_all','skappa_sole','skappa_soleshared']
df=pd.read_parquet(f_kappas_combined,columns=col_list)
df=df[df.quarter>'1999-01-01']


# %%
df[df.quarter<'2011-01-01'].groupby(['quarter']).mean()[['kappa','kappa_sole','kappa_soleshared']].plot(figsize=(20,10))
plt.xlabel("")
...
# %%
df[(df.quarter>'2013-09-30')&(~df.skappa.isnull())].groupby(['quarter']).mean()[['skappa','skappa_sole','skappa_soleshared']].plot(figsize=(20,10))
plt.xlabel("")
plt.legend(['All Shares','Sole Voting Rights','Sole+Shared Voting Rights'])
plt.ylim(0, 1)
