...
mgr_tots = df.groupby(['quarter', 'permno'])['mgrno'].nunique(
).reset_index().groupby(['quarter'])['mgrno'].describe(percentiles=[.1, .5])
mgr_totsS = dfs.groupby(['quarter', 'permno'])['mgrno'].nunique(
).reset_index().groupby(['quarter'])['mgrno'].describe(percentiles=[.1, .5])

# %%
# 2 minutes# calculate number of firms
a = df[(df.sharecode_drop == False) & (df.permno_drop == False)
       ].groupby(['quarter'])['permno'].nunique()
b = df.groupby(['quarter'])['permno'].nunique()
c = dfs[(dfs.sharecode_drop == False) & (dfs.permno_drop == False)
        ].groupby(['quarter'])['permno'].nunique()
...
# Figure 2
plt.clf()
ax = nfirms[['TR (Restricted)', 'TR (Unrestricted)']].plot(
    figsize=(20, 10), color=['navy', 'maroon'])
#plt.axhline(y=500,color='0.75',linestyle = '--')
nfirms[['Scraped (Restricted)', 'Scraped (Unrestricted)']
       ].plot(ax=ax, color=['navy', 'maroon'])
plt.xlabel("")
...
mgr_tots[['mean', '50%', '10%', 'min']].plot(
    figsize=(20, 10), ax=ax, style='-', color=['b', 'r', 'y', 'g'])
mgr_totsS[['mean', '50%', '10%', 'min']].plot(
    figsize=(20, 10), ax=ax, style='--', color=['b', 'r', 'y', 'g'])
plt.xlabel("")
...
dfs = dfs[(dfs.permno_drop == False) & (
    dfs.sharecode_drop == False) & (dfs.beta < 0.5)]

a = (100 * df.groupby(['permno', 'quarter'])
     ['beta'].sum()).groupby(level=1).mean()
b = (100 * dfs.groupby(['permno', 'quarter'])
     ['beta'].sum()).groupby(level=1).mean()
pd.concat([a, b], axis=1).plot(figsize=(20, 10), style=[
    '-', '--', ], color=['navy', 'maroon'])
