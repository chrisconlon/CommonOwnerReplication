...
df3 = pd.concat([df.groupby('quarter').apply(wavg_l1),
                 df.groupby('quarter').apply(wavg_l2)], axis=1)
...
df4 = pd.concat([df2.groupby('quarter').apply(wavg_l1),
                 df2.groupby('quarter').apply(wavg_l2)], axis=1)
...
ax = pd.concat([df3,
                df4],
               axis=1)[['investor_l2',
                        'l2_drop_blackrockvanguard',
                        'investor_l1',
                        'l1_drop_blackrockvanguard']].plot(figsize=(20,
                                                                    10),
                                                           color=['navy',
                                                                  'navy',
                                                                  'maroon',
                                                           ...
                                                           style=['-',
                                                                  ...
                                                                  '-.',
                                                                  ':'])
plt.legend(['Investor Similarity $(L_2)$',
            'Investor Similarity $(L_2)$ (No BlackRock/Vanguard)',
            'Investor Similarity $(L_1)$',
            'Investor Similarity $(L_1)$ (No BlackRock/Vanguard)'])
plt.xlabel("")
plt.ylabel("Investor Similarity (AUM Weighted)")
plt.ylim(0, 1)
