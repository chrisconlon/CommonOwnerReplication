...
resid_cs = pyhdfe.create(
    df_kappas2[['quarter_fe']].values).residualize(variables)
resid_ts = pyhdfe.create(df_kappas2[['pair_fe']].values).residualize(variables)
resid_pa = pyhdfe.create(
    df_kappas2[['pair_fe', 'quarter_fe']].values).residualize(variables)

# Do the Variance Decomposition for each case
tab_mat = np.vstack([do_decomp(variables), do_decomp(
    resid_cs), do_decomp(resid_ts), do_decomp(resid_pa)]) * 100.0
...
table3.iloc[:, 0:2].to_latex(
    f_table3, float_format=lambda x: '%.2f' % x + str('%'), column_format='l cc')
