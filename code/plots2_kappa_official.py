...
def weighted(x, cols):
    ...
    a4=np.average(x[cols].values, weights=x['mkt_cap_from'].values, axis=0)[0]
...
total_df=pd.merge(
    pd.merge(df,ihhi,left_on=['from','quarter'],right_on=['permno','quarter'],how='left'),
...
total_df['w_gmean']=gmean([total_df['mkt_cap_from'],total_df['mkt_cap_to']],axis=0)
total_df['w_s_gmean']=gmean([total_df['saleq_x'],total_df['saleq_y']],axis=0)
...
df_cosine=total_df.groupby(['quarter'])['cosine'].describe( percentiles = [0.05,0.25, 0.5,0.75, 0.95 ] )

# Percentiles of Kappa and IHHI
kappa_pct=df.groupby(['quarter'])['kappa'].describe( percentiles = [0.05,0.25, 0.5,0.75, 0.95 ])
ihhi_pct=ihhi[~ihhi.ihhi.isnull()].groupby(['quarter'])['ihhi'].describe( percentiles = [0.05,0.25, 0.5,0.75, 0.95 ])

# drop k_ff =1 cases for tunneling
tunnel_df=(df[df['from']!= df['to']].set_index('quarter')[['kappa_sqrt','kappa','kappa_pow2','kappa_pow3']]>1).groupby(level=0).mean()
tunnel_df2=(df[df['from']!= df['to']].set_index(['from','quarter'])[['kappa_sqrt','kappa','kappa_pow2','kappa_pow3']]>1).groupby(level=[0,1]).max()
...
# Appendix Figure 13
plt.clf()
qtr_mean[['kappa','kappa_sqrt','kappa_pow2','kappa_pow3']].plot(figsize=(20,10))
...
plt.clf()
total_df[(total_df.same_sic==True)].groupby(['quarter'])['kappa'].mean().plot(figsize=(20,10))
total_df[(total_df.same_sic==False)].groupby(['quarter'])['kappa'].mean().plot()
#plt.title("Average Pairwise Profit Weights $(\kappa)$ Within and Between SIC code")
plt.xlabel("")
plt.ylabel("$\kappa$ weight")
plt.ylim(0, 1)
plt.legend(["$\kappa$ same SIC","$\kappa$ different SIC"])
...
plt.legend(['95th percentile','75th percentile','50th percentile','25th percentile','5th percentile'])
plt.ylabel("$\kappa$ Quantiles")
plt.xlabel("")
plt.ylim(0, 1)
...
# Figure 7
total_df.groupby(['quarter'])[['kappa','cosine','l1_measure']].mean().plot(figsize=(20,10))
plt.xlabel("")
#plt.title("Cosine Similarity and $\kappa$")
plt.ylim(0, 1)
plt.legend([r'$\kappa_{f,g}$', r'$L_2$ similarity $cos(\beta_f,\beta_g)$',r'$L_1$ similarity $|\beta_f - \beta_g|$'])
...
# Figure 14a
df_firm[['quarter','ihhi','chhi_05','chhi_2','chhi_3','chhi_4']].groupby(['quarter']).mean().plot(figsize=(20,10))
plt.xlabel("")
plt.ylabel("Effective Control HHI")
plt.ylim(0, 3500)
plt.legend([r'$\gamma = \beta$',r'$\gamma \propto \sqrt{\beta}$',r'$\gamma \propto \beta^2$',r'$\gamma \propto \beta^3$',r'$\gamma \propto \beta^4$'])
plt.savefig(f_chhi,bbox_inches="tight")
# %%


# Figure 14b
df_firm[['quarter','ihhi','chhi_05']].groupby(['quarter']).mean().plot(figsize=(20,10))
plt.xlabel("")
plt.ylabel("Effective Control HHI")
plt.ylim(0, 350)
...
plt.ylabel("Percentage of $\kappa$ > 1")
plt.legend('')
plt.ylim(0, 12)
#plt.legend([r'$\gamma = \beta$',r'$\gamma \propto \sqrt{\beta}$',r'$\gamma \propto \beta^2$',r'$\gamma \propto \beta^3$'])
...
# %%
(100.0*tunnel_df[['kappa','kappa_sqrt','kappa_pow2','kappa_pow3']]).plot(figsize=(20,10))
plt.xlabel("")
#plt.title("Potential Tunneling")
plt.ylabel("Percentage of $\kappa$ > 1")
plt.ylim(0, 20)
