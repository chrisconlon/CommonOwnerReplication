...
df_scrape_holdings = 100*df_scrape_subset.groupby(['quarter', 'permno']).sum()
...
df_tr_holdings['beta'].unstack().plot(ax=ax,color=['navy','maroon','darkgreen'],style=['-','-','-'])
matplotlib.rc('xtick', labelsize=24) 
matplotlib.rc('ytick', labelsize=24) 
plt.ylabel("Percent of Shares Reported in 13F Filings", {'size':'24'})

df_scrape_holdings['beta'].unstack().plot(ax=ax, style=['--','--','--'],color=['navy','maroon','darkgreen'])
plt.xlabel("")
