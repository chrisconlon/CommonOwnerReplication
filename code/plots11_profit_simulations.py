...
def run_several(kappa_list,maverick=False):
    mylist=[run_simulation(n_firms,betas,gammas,k,maverick) for k in kappa_list]
    df_out=pd.DataFrame(mylist)
    df_out.columns=['inside_share','prices','og_diversion','own_elas','total_pi']
    df_out.index=df.index
    return df_out

# %%
(inside_share,prices,og_diversion,own_elas,pi_1)=run_simulation(n_firms,betas,gammas,kappa=0.21)
(inside_share2,prices2,og_diversion2,own_elas2,pi_2)=run_simulation(n_firms,betas,gammas,kappa=0.7)
(base_share,base_prices,base_og,base_elas,pi_base)=run_simulation(n_firms,betas,gammas,kappa=0)
...
df_out2.columns=['inside_mav','prices_mav','og_diversion_mav','own_elas_mav','total_pi_mav']
...
# %%
df_jj=pd.read_csv(f_jj_markups,low_memory=False,names=['year','jj_markup'])
...
df2[['prices','jj_markup','prices_mav']].plot(figsize=(20,10),color=['black','navy','maroon'])
plt.legend(['Common Ownership Markups','DeLoecker, Eeckhout, Unger (2020)','Common Ownership (w/ maverick)'], prop={'size': 24},loc='upper left')
plt.xlabel('')
plt.ylabel('Markup over Cost',size=24)
plt.ylim(1,1.65)
