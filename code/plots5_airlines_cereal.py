...
df_firms2=df_firms.loc[df_firms['siccd']==6021,['permno','quarter','comnam']].copy()
...
# %%
df_tot=pd.concat([df_cereal.groupby(['quarter'])['kappa'].median(),df_airlines.groupby(['quarter'])['kappa'].median(),df_banks.groupby(['quarter'])['kappa'].median()],axis=1)

# %%
df_tot[df_tot.index>'1999-01-01'].plot(figsize=(20,10),color=['navy','maroon','darkgreen'])
plt.legend(['RTE Cereal','Airlines','Banks'])
plt.ylabel("Median Pairwise Profit Weights $(\kappa)$")
plt.xlabel("")
plt.ylim(0, 1)
