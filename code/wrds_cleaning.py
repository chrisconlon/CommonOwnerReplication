...
def blackrock_fix(df):
    ...
    df.loc[(df.mgrno.isin([7900, 9385])) & (
        df.rdate != df.fdate), 'blackrock_fix'] = True
    df.loc[(df.mgrno.isin([7900, 9385])), 'rdate'] = df.loc[(
        df.mgrno.isin([7900, 9385])), 'fdate']
    return df
...
def filter_s34(df, df_sp):
    return pd.merge(df, df_sp, left_on=['cusip', 'rdate'], right_on=[
                    'cusip', 'qdate'], how='inner').drop(columns=['qdate'])
...
def consolidate_mgrs(main_df, f_consolidation):
    ...
    x['mgrno'] = x['mgrno_to']
    part2 = x.drop(columns=['mgrno_from',
                            'mgrno_to']).groupby(['permno',
                                                  'cusip',
                                                  'mgrno',
                                                  'quarter',
                                                  'fdate']).agg({'shares': sum,
                                                                 'prc': max,
                                                                 'shrout1': max,
                                                                 'shrout2': max,
                                                                 'sole': sum,
...
def add_drops(df, f_drops, df_names2):
    keep_cols = list(set(list(df.columns) +
                         ['no_managers', 'permno_drop', 'sharecode_drop']))
    # count number of managers
    df['no_managers'] = df.groupby(['permno', 'quarter'])[
        'mgrno'].transform('nunique')
    # dual class shares
    ...
    df.loc[(df['quarter'] >= df['start']) & (
        df['quarter'] <= df['end']), 'permno_drop'] = True

    df = pd.merge(df, df_names2[['permno', 'quarter', 'shrcd']].drop_duplicates(), on=[
                  'permno', 'quarter'])
    # ADR's and REITs
    df.loc[~df.shrcd.isin([10, 11, 12, 18]), 'sharecode_drop'] = True
    df[['permno_drop', 'sharecode_drop']] = df[[
        'permno_drop', 'sharecode_drop']].fillna(False)
    return df[keep_cols]
...
def add_stock_splits(df, df_msf):
    ...
    merged.loc[(merged.cfacshr_x != merged.cfacshr_y) & (
        ~merged.cfacshr_x.isnull()) & (~merged.cfacshr_y.isnull()), 'is_split'] = 1
    merged['is_split'].fillna(0, inplace=True)
    merged['share_split'] = merged.groupby(['permno', 'mgrno', 'rdate'])[
        'is_split'].transform(max)
    return merged


def construct_fundamentals(df_fund, df_names2):
    return pd.merge(df_names2[['permno', 'quarter']], df_fund.rename(columns={'datadate': 'quarter'}), on=['permno', 'quarter'], how='left')[['permno', 'quarter', 'oibdpq', 'atq', 'niq',
                                                                                                                                              'saleq', 'cogsq']].drop_duplicates()


def construct_bus_segments(df_seg, df_sp500):
    df_spe = expand_splist(df_sp500).rename(columns={'qdate': 'quarter'})
    z = pd.merge(df_spe, df_seg.groupby(['permno', 'quarter'])['stype'].max().reset_index(
    ), on=['permno', 'quarter'], how='left').sort_values(['permno', 'quarter'])
...
def expand_names(df_names, df_sp500):
    ...
    return x[(x['qdate'] >= x['namedt']) & (x['qdate'] <= x['nameenddt'])].drop(columns=[
        'namedt', 'nameenddt', 'st_date', 'end_date', 'final_date']).rename(columns={'qdate': 'quarter'})


def expand_splist(df_sp):
    df_sp['key'] = 0
    alldates = pd.DataFrame({'qdate': pd.date_range(
        '01-01-1980', pd.to_datetime('today'), freq='Q')})
    alldates['key'] = 0
    x = pd.merge(df_sp, alldates, on='key')
    return x[(x['qdate'] >= x['start']) & (
        x['qdate'] <= x['ending'])][['permno', 'qdate']]
#
...
def dedup_s34(df):
    ...
    part2 = dups_df.loc[dups_df.min_shares == dups_df.max_shares,
                        idx + data2].groupby(idx).last().reset_index()

    # Choosing different Fdates gives different answers -- these are more
    # challenging
    ...
    part3 = problems[problems.share_split == 1].groupby(idx).first()[
        data2].reset_index()

    # If a not split, take the last fdate for each rdate (this is riskier)
    part4 = problems[problems.share_split == 0].groupby(idx).last()[
        data2].reset_index()
    ...
    print("Observations with known split (take first): ", len(
        part3), len(problems[problems.share_split == 1]))
    print("Other observations (update?) (take last): ", len(
        part4), len(problems[problems.share_split == 0]))

    return pd.concat([part1, part2, part3, part4]).rename(
        columns={'rdate': 'quarter'})
...
def compute_betas(df, df_msf):
    ...
    y = pd.merge(df, df_msf[['permno', 'qdate', 'prc', 'shrout']], left_on=[
                 'permno', 'quarter'], right_on=['permno', 'qdate'], how='left')
    y.loc[:, ['shrout1', 'shrout2', 'shrout']] = y[[
        'shrout1', 'shrout2', 'shrout']].replace(0, np.nan)

    y['med_price'] = y.groupby(['permno', 'quarter'])[
        'prc_x'].transform(np.median)
    y['med_shares'] = y.groupby(['permno', 'quarter'])['shrout2'].transform(np.median).combine_first(
        1e3 * y.groupby(['permno', 'quarter'])['shrout1'].transform(np.median))
    ...
    y = alt_betas(y)
    return y[['permno', 'mgrno', 'quarter', 'shares', 'shares_outstanding',
              'price', 'beta', 'beta_sole', 'beta_soleshared', 'sole', 'shared', 'no']]
...
def process_scraped(fn_scrape, fn_big4):
    ...
    df = df.rename(
        columns={
            ...
            'none': 'no'}).drop(
        columns=['rdate'])
    return alt_betas(pd.merge(df, pd.read_csv(
        fn_big4), how='left', on=['mgrno']))
...
def alt_betas(y):
    ...
    y['beta_soleshared'] = (y['shares'] - y['no']) / \
        (1e3 * y['shares_outstanding'])
    y['beta_sole'] = (y['shares'] - y['no'] - y['shared']) / \
        (1e3 * y['shares_outstanding'])
    return y
...
def combine_betas(df, dfs, cut_date='2000-01-01'):
    ...
    return pd.concat([df[df.quarter <= cut_date], dfs.loc[dfs.quarter >
                                                          cut_date, cols]], axis=0, ignore_index=True)
