...
def beta_to_kappa(df):
    ...
    total_df[['kappa',
              ...
              'cosine',
              'kappa_sole',
              'kappa_soleshared']] = total_df[['kappa',
                                               'kappa_CLWY',
                                               'kappa_pow2',
                                               'kappa_pow3',
                                               'kappa_sqrt',
...
def do_one_robustness(df):
    [betas_soleshared, mgr_keys, permno_keys] = matlab_sparse(
        df.mgrno, df.permno, df.beta_soleshared, compress=False)
    [betas_sole, mgr_keys, permno_keys] = matlab_sparse(
        df.mgrno, df.permno, df.beta_sole, compress=False)

    [betas, mgr_keys, permno_keys] = matlab_sparse(
        df.mgrno, df.permno, df.beta, compress=False)
...
def do_one_merger_breakup(df2):
    ...
    [betas, mgr_keys, permno_keys] = matlab_sparse(
        df2.mgrno, df2.permno, df2.beta)
    k1 = calc_kappa(betas)

    # now do the breakup case using the augmented data
    [betas_b, mgr_keys_b, permno_keys_b] = matlab_sparse(
        df3.mgrno, df3.permno, df3.beta)
    ...
    [betas_m, mgr_keys_m, permno_keys_m] = matlab_sparse(
        df4.mgrno_merger, df4.permno, df4.beta)
    k3 = calc_kappa(betas_m)

    # Ignore BlackRock+Vanguard
    df4 = df2[~(df2['InvestorName'].isin(['BlackRock', 'Vanguard']))]
    [betas_drop, mgr_keys_drop, permno_keys_drop] = matlab_sparse(
        df4.mgrno, df4.permno, df4.beta, compress=False)
...
# input: long dataframe of Manager, Firm, Beta_fs
# Output: long dataframe of Quarter, Firm_from, Firm_to, kappa_fg, ihhi_f,
# ihhi_g, cosine_fg
...
# Subtract beta_f from each column of beta and sum of absolute deviations,
# stack for L1.
...
#    y=applyParallel(df[['permno','quarter','beta','inv_total']].groupby(['permno','quarter']), agg_chhi)
    ...
    x = df.groupby(['permno', 'quarter']).agg(
        {'shares_outstanding': np.max, 'price': np.median})
...
def agg_chhi(x):
    ...
    return pd.Series(names, index=['retail_share', 'ihhi', 'chhi_05',
                                   'chhi_2', 'chhi_3', 'chhi_4', 'chhi_clwy', 'chhi_clwy2'])
...
# Options: Gamma 'CLWY', 'default' (Rotemberg), numeric: convexity
# parameter "a" for gamma=beta^a
