...
final_df = pd.merge(total_dft, fix_scrape_cols(total_dfs),
                    on=['from', 'to', 'quarter'], how='outer')
...
firm_similarity = df.groupby(['quarter']).apply(
    do_one_firm_similarity).reset_index(drop=True)
