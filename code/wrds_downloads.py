...
def get_names(db):
	return fix_ending_dates(clean_wrds(db.get_table('crsp','stocknames')), 'nameenddt',['permno'])
...
def get_crosswalk(db,permno_list):
	...
	crosswalk=clean_wrds(crosswalk[~(crosswalk.linkenddt<'1980-01-01') & crosswalk.lpermno.isin(permno_list)])
...
def get_fundamentals(db,crosswalk):
    fields=['gvkey','datadate','fyearq','fqtr','fyr','datafqtr','indfmt','cusip','oibdpq','atq','niq','saleq','cogsq']
    query = "select " + ', '.join(fields)  +" from comp.fundq where fyearq> 1979 and gvkey in %s" % repr(tuple(crosswalk.gvkey.unique()))
    ...
    # note: this is rare and only happens when fiscal year changes, taking first has no effect
    df_fundq2=df_fundq.groupby(['gvkey','datadate']).last().reset_index()
    
    # merge in the gvkey-permno crosswalk
    x=pd.merge(df_fundq2,crosswalk[['gvkey','lpermno','linkdt','linkenddt']].drop_duplicates(),on='gvkey').rename(columns={'lpermno':'permno'})
    y=x[(x.datadate >= x.linkdt)&(x.datadate <= x.linkenddt)].copy()
    return clean_wrds(y.sort_values('linkenddt').groupby(['permno','datadate']).last().reset_index()[fields+['permno']])

## Download the MSF file from CRSP
# - convert to quarterly data by taking last observation
def get_msf(db,permno_list,trim=False):
	fields=['cusip','permno','hsiccd','date','prc','altprc','shrout','altprcdt','cfacshr']
	query = "select " + ', '.join(fields)  +" from crsp.msf where date > '1979-12-31' and permno in %s" % repr(tuple(permno_list))
	...
	# Trim the MSF data for only dates and permnos in the S&P at the time
		...
		return df_msf3[(df_msf3['date'] >= df_msf3['start'])&(df_msf3['date']<= df_msf3['ending'])]
	else:
		return df_msf2
...
def get_short_interest(db,crosswalk):
	...
	short_int3=short_int2[(short_int2.datadate<=short_int2.linkenddt)&(short_int2.datadate>=short_int2.linkdt)].copy()
	return convert_to_quarter(short_int3,'datadate',['lpermno'])[['lpermno','lpermco','qdate','gvkey','iid','shortint','shortintadj','datadate','splitadjdate']]
...
def get_segments(db,crosswalk):
    fields=['gvkey','stype','datadate','naicss1','naicss2','naicss3','sics1','sics2','sics3']
    query = "select " + ', '.join(fields)  +" from comp_segments_hist.wrds_segmerged where stype ='BUSSEG'"
    df=db.raw_sql(query)
    ...
    x=pd.merge(df,crosswalk[['gvkey','lpermno','linkdt','linkenddt']],on='gvkey').rename(columns={'lpermno':'permno'})
    return clean_wrds(x[(x.datadate >= x.linkdt)&(x.quarter <= x.linkenddt)].copy())[['permno','quarter','datadate','stype']]
...
def convert_to_quarter(df,date_name,group_ids):
    ...
    df['qdate']=df[date_name] - pd.tseries.offsets.DateOffset(days=1) + pd.tseries.offsets.QuarterEnd()
...
def fix_ending_dates(df,date_field,group_list):
    ...
    df.loc[df[date_field]==df.final_date,date_field] = df.loc[df[date_field]==df.final_date,date_field] - pd.tseries.offsets.DateOffset(days=1) + pd.tseries.offsets.QuarterEnd()
    return df