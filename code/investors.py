...
def compute_investor_info(df, f_big4):
    tmp = df.groupby(['quarter']).apply(
        do_one_investor_similarity).reset_index(drop=True)
    return pd.merge(tmp, pd.read_csv(f_big4), how='left', on=['mgrno'])


def calc_big4(df, big4):
    df2 = pd.merge(df, big4, on=['mgrno'], how='inner').groupby(
        ['quarter', 'permno', 'InvestorName'])['beta'].sum().unstack()
    df2.columns = [
        'beta_BlackRock',
        'beta_Fidelity',
        'beta_StateStreet',
        'beta_Vanguard']
    return df2[['beta_BlackRock', 'beta_Vanguard',
                'beta_StateStreet', 'beta_Fidelity']].fillna(0)
...
def do_one_investor_similarity(df):
    ...
    out_df = pd.DataFrame({'mgrno': mgr_keys.astype(int),
                           'aum_weight': aum,
                           'l2_similarity': l2,
                           'l1_similarity': l1,
                           'cov_aum_l1': np.cov(l1,
                                                aum)[1][0]})
...
def do_one_firm_similarity(df):
    ...
    out_df = pd.DataFrame({'permno': permno_keys.astype(int),
